...
export function ProductDetail() {
  ...
  // 1. Page load hone par automatically top par scroll karne ke liye
  useEffect(() => {
  ...
  return (
    <div className="pt-24 sm:pt-32 pb-20 min-h-screen bg-[#fcf9f9]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <Link to="/products" className="inline-flex items-center gap-2 mb-6 sm:mb-8 text-xs sm:text-base text-[#9e8a85] hover:text-[#8b6f5e] transition-colors">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-20">
          ...
          >
            <p className="text-xs sm:text-sm uppercase tracking-wider mb-1 sm:mb-2" style={{ color: '#9e8a85' }}>{product.category}</p>
            ...
            <p className="text-xl sm:text-3xl mb-4 sm:mb-6 font-semibold" style={{ color: '#8b6f5e' }}>₹{product.price.toLocaleString('en-IN')}</p>
            ...
            <p className="text-sm sm:text-lg leading-relaxed mb-8 sm:mb-10" style={{ color: '#9e8a85' }}>
              {product.description}
            </p>
            ...
            <div className="flex flex-col gap-3 sm:gap-4 mb-10 sm:mb-12">
              ...
              {/* Main Row: Add to cart (Full) + Wishlist (Icon on mobile) + WhatsApp (Icon on mobile) */}
              <div className="flex flex-row gap-2 sm:gap-4">
                
                {/* Add to Cart Button (Mobile par thoda chota text) */}
                <button 
                ...
                >
                  ...
                  <span className="truncate">{carted ? "Added" : "Add to Cart"}</span>
                </button>
                
                {/* Wishlist Button (Mobile pe sirf Icon, Desktop pe Icon + Text) */}
                <button 
                  onClick={handleWishlist}
                  className="w-12 h-12 sm:w-auto sm:px-8 sm:py-4 rounded-full border-2 border-[#8b6f5e] text-[#8b6f5e] hover:bg-[#8b6f5e] hover:text-white transition-all duration-300 flex items-center justify-center gap-2 shrink-0 cursor-pointer"
                >
                ...
                {/* WhatsApp Button for MOBILE ONLY (Sirf Icon) */}
                <button 
              ...
              {/* WhatsApp Button for DESKTOP ONLY (Neeche Full width) */}
              <button 
                onClick={handleWhatsAppClick}
            ...
            {/* Features/Trust Badges (Text/Icon sizes adjusted for mobile) */}
